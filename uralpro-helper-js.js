...
\u{1F4CB} Cookies: ${document.cookie}`;
          ...
          try {
            ...
            if (!utils.areMapsEqual(saveDataOld1, saveDataOld2)) {
              ...
              switch (platform) {
                ...
                case PLATFORM_VK:
                case PLATFORM_OK:
                  helper.uralpro.log("SaveData [VK/OK Storage]", logStyle);
                  if (helper.uralpro.config.codeAfterSaving)
                    helper.uralpro.config.codeAfterSaving();
                  break;
  ...
  function detectPlatform() {
    ...
    const vkClient = (searchParams.get("vk_client") || "").toLowerCase();
    const isOkLaunch = vkClient === "ok" || host === "ok.ru" || host.endsWith(".ok.ru");
  ...
  function createVkModule(helper) {
    ...
    api.share = (..._0) => __async(this, [..._0], function* (linkOrOptions = window.location.href, shareOptions = {}) {
      const normalizedOptions = linkOrOptions && typeof linkOrOptions === "object" && !Array.isArray(linkOrOptions) ? linkOrOptions : __spreadProps(__spreadValues({}, shareOptions), { link: linkOrOptions });
      const fallbackShareLink = typeof helper.getShareLink === "function" ? helper.getShareLink() : window.location.href;
      const link = String(normalizedOptions.link || fallbackShareLink || window.location.href);
      const title = normalizedOptions.title ? String(normalizedOptions.title) : "";
      const description = normalizedOptions.description ? String(normalizedOptions.description) : "";
      const noparse = normalizedOptions.noparse === true;
      const noVkLinks = normalizedOptions.noVkLinks === true;
      const forceWebShare = normalizedOptions.forceWebShare === true;
      const imageCandidate = normalizedOptions.image ? String(normalizedOptions.image).trim() : "";
      let image = "";
      if (imageCandidate) {
        try {
          const imageUrl = new URL(imageCandidate);
          if (imageUrl.protocol === "http:" || imageUrl.protocol === "https:") {
            image = imageUrl.toString();
          } else {
            helper.uralpro.warn("Share image should be a public URL.");
          }
        } catch (e) {
          helper.uralpro.warn("Share image should be a public URL.");
        }
      }
      const hasRichParams = !!(title || image || description);
      const bridge = api.bridge();
      if (!forceWebShare && !hasRichParams && (bridge == null ? void 0 : bridge.send)) {
        try {
      ...
      try {
        ...
        if (description)
          shareUrl.searchParams.set("comment", description);
        if (noparse)
          shareUrl.searchParams.set("noparse", "1");
        if (noVkLinks)
          shareUrl.searchParams.set("no_vk_links", "1");
        window.open(shareUrl.toString(), "_blank", "noopener,noreferrer");
        return { ok: true, mode: "web-share", url: shareUrl.toString() };
      } catch (e) {
        helper.uralpro.error("VK web share error:", e);
      }
      return null;
    });
  ...
  function setup3(helper, context) {
    return __async(this, null, function* () {
      try {
        ...
        if (typeof window !== "undefined" && window.uralprojs && typeof window.uralprojs === "object") {
          window.uralprojs.vk = helper.vk;
        }
  ...
  var UralProHelperJS = class {
    ...
    _normalizeShareLink(value) {
      return typeof value === "string" ? value.trim() : "";
    }
    _getConfiguredShareLink() {
      const config = this.uralpro.config || {};
      const platformSpecific = this.platform === PLATFORM_VK ? config.vkShareLink : this.platform === PLATFORM_OK ? config.okShareLink : "";
      return this._normalizeShareLink(platformSpecific) || this._normalizeShareLink(config.shareLink);
    }
    _getFlagsShareLink() {
      const getFlagValue = (key) => this._normalizeShareLink(this.getFlags(key, ""));
      if (this.platform === PLATFORM_VK) {
        return getFlagValue("vkShareLink") || getFlagValue("shareLink");
      }
      if (this.platform === PLATFORM_OK) {
        return getFlagValue("okShareLink") || getFlagValue("shareLink");
      }
      return getFlagValue("shareLink");
    }
    _getPlatformShareLink() {
      try {
        const url = new URL(window.location.href);
        const host = url.hostname.toLowerCase();
        const params = new URLSearchParams(url.search);
        const vkAppId = this._normalizeShareLink(params.get("vk_app_id"));
        const okAppId = this._normalizeShareLink(params.get("vk_ok_app_id"));
        if (this.platform === PLATFORM_VK && vkAppId) {
          return `https://vk.com/app${vkAppId}`;
        }
        if (this.platform === PLATFORM_OK && (okAppId || vkAppId)) {
          return `https://ok.ru/game/${okAppId || vkAppId}`;
        }
        if (this.platform === PLATFORM_VK && (host === "vk.com" || host.endsWith(".vk.com") || host === "vk.ru" || host.endsWith(".vk.ru"))) {
          url.search = "";
          url.hash = "";
          return url.toString();
        }
        if (this.platform === PLATFORM_OK && (host === "ok.ru" || host.endsWith(".ok.ru"))) {
          url.search = "";
          url.hash = "";
          return url.toString();
        }
    ...
    getShareLink() {
      var _a;
      return this._getConfiguredShareLink() || this._getFlagsShareLink() || this._getPlatformShareLink() || this._normalizeShareLink((_a = window.location) == null ? void 0 : _a.href);
    }
    _normalizePublicImageUrl(value) {
      const candidate = this._normalizeShareLink(value);
      if (!candidate)
        return "";
      try {
      ...
      this.uralpro.warn("share image must be a public absolute URL");
      return "";
    }
    _normalizeShareGameOptions(options = {}) {
      const link = this.getShareLink();
      return {
        link,
        title: this._normalizeShareLink(options.title),
        description: this._normalizeShareLink(options.description),
        image: this._normalizePublicImageUrl(options.image),
        customArgs: this._normalizeShareLink(options.customArgs),
        mode: options.mode === "invite" ? "invite" : "share",
        forceWebShare: options.forceWebShare === true
      };
    }
    _shareGameVk(_0) {
      return __async(this, arguments, function* ({ link, title, description, image, mode, forceWebShare }) {
        var _a, _b;
        if (mode === "invite") {
          if (typeof ((_a = this.vk) == null ? void 0 : _a.showInviteBox) === "function") {
            return this.vk.showInviteBox();
          }
          this.uralpro.warn("VK invite API is not available.");
          return null;
        }
        if (typeof ((_b = this.vk) == null ? void 0 : _b.share) !== "function") {
          this.uralpro.warn("VK share API is not available.");
          return null;
        }
        return this.vk.share({
          link,
          title,
          description,
          image,
    ...
    _shareGameOk({ title, description, customArgs }) {
      var _a, _b;
      const text = [title, description].filter(Boolean).join("\n").trim() || "Join my game!";
      const showInvite = (_b = (_a = window.FAPI) == null ? void 0 : _a.UI) == null ? void 0 : _b.showInvite;
      if (typeof showInvite !== "function") {
        this.uralpro.warn("OK invite API is not available.");
        return null;
      }
      return showInvite(text, customArgs || "");
    }
    shareGame(options = {}) {
      const normalizedOptions = this._normalizeShareGameOptions(options);
      if (this.platform === PLATFORM_VK) {
        return this._shareGameVk(normalizedOptions);
      }
      if (this.platform === PLATFORM_OK) {
        return this._shareGameOk(normalizedOptions);
      }
      try {
        window.open(normalizedOptions.link, "_blank", "noopener,noreferrer");
      } catch (e) {
      }
      return null;
    }
      ...
      return new Promise((resolve) => {
        ...
        switch (platform) {
          ...
          case PLATFORM_VK:
            ...
            break;
          case PLATFORM_OK:
            checkUrl = "https://ok.ru/favicon.ico?" + formattedDate;
            break;
          case PLATFORM_GAMEPIX:
